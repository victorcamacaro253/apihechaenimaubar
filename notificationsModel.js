   ...
   try {
    ...
    async deleteNotification(id){
     ...
     async getUserNotification(username){
        ...
        try {
            const result = await query('SELECT message,nombre,apellido,datetime FROM notificaciones_usuarios INNER JOIN notificaciones ON notificaciones_usuarios.id_notificacion=notificaciones.id INNER JOIN usuario ON notificaciones_usuarios.id_usuario=usuario.id WHERE usuario.nombre = ?',[username])
